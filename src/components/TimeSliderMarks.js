...
// };
export default function TimeSliderMarks({
   duration,
   ChangeCurrentTime,
   currentTime,
}) {
   ...
   //        label: `${minute}:${secondLeft < 10 ? `0${secondLeft}` : secondLeft}`,
   //     },
   //  ];
   return (
      <Box sx={{}}>
         <StyledSlider
            ...
            value={currentTime}
            onChange={(_, value) => ChangeCurrentTime(value)}
            // onChange={(Event,value,activeThumb)=>{
            //   activeThumb = currentTime
            // }}
         />
      </Box>
   );
